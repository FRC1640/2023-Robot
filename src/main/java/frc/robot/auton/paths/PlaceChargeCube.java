...
public class PlaceChargeCube {
  ...
  List<PathPlannerTrajectory> pathGroup = PathPlanner.loadPathGroup("RollerChargeCube", new PathConstraints(1.5, 1.5));

  public PlaceChargeCube(){
  ...
  public CommandBase loadAuto(Gyro gyro, DriveSubsystem swerve, ArmSubsystem armSubsystem, GrabberSubsystem grabberSubsystem, WristSubsystem wristSubsystem) { 
    ...
    return Commands.sequence(gyroCommand, setConeMode, new EndPitch2(swerve, gyro, 2400).deadlineWith(auto), new Balance(swerve, gyro));
  }
}
