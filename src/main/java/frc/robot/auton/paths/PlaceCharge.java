...
public class PlaceCharge {
  ...
  List<PathPlannerTrajectory> pathGroup = PathPlanner.loadPathGroup("RollerCharge", new PathConstraints(1.5, 2));
  ...
  public CommandBase loadAuto(Gyro gyro, DriveSubsystem swerve, ArmSubsystem armSubsystem, GrabberSubsystem grabberSubsystem) { 
    ...
    return Commands.sequence(gyroCommand, setConeMode, new EndPitch2(swerve, gyro, 2000).deadlineWith(auto), new Balance(swerve, gyro));
  }
}
