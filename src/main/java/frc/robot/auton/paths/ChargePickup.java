...
public class ChargePickup {
  ...
  List<PathPlannerTrajectory> pathGroup = PathPlanner.loadPathGroup("ChargePickupNew", new PathConstraints(1.5, 2));
  ...
  public CommandBase loadAuto(Gyro gyro, DriveSubsystem swerve, ArmSubsystem armSubsystem, GrabberSubsystem grabberSubsystem) { 
    ...
    return Commands.sequence(gyroCommand, setConeMode, new EndPitch2(swerve, gyro, 2100).deadlineWith(auto), new Balance(swerve, gyro));
  }
}
